...
impl Default for LunaApi {
    ...
    fn get_doc(&self, id: DocumentId) -> Result<&Document, LunaError> {
        self.documents.get(&id).ok_or(LunaError::DocumentNotFound(id))
    }

    fn get_doc_mut(&mut self, id: DocumentId) -> Result<&mut Document, LunaError> {
        self.documents.get_mut(&id).ok_or(LunaError::DocumentNotFound(id))
    }
    ...
    fn doc_path_to_string(doc: &Document) -> Option<String> {
        doc.path().map(|path| Self::path_to_string(path))
    }
    ...
    fn ensure_not_already_open(&self, canonical: &Path) -> Result<(), LunaError> {
        if self.documents.values().any(|doc| {
            matches!(doc.path(), Some(doc_path) if doc_path == canonical)
        }) {
            return Err(LunaError::DocumentAlreadyOpen(Self::path_to_string(canonical)));
        }
        Ok(())
    }
    ...
    pub fn open_file(&mut self, path: &str) -> Result<DocumentId, LunaError> {
        let canonical = Self::canonicalize_path(path)?;
        self.ensure_not_already_open(&canonical)?;
        let content = std::fs::read_to_string(&canonical).map_err(|e| LunaError::io(path, e))?;
        let id = self.next_id();
        let doc = Document::from_disk(id, canonical.clone(), &content);
        self.documents.insert(id, doc);
        self.emit_file_opened(id, Some(Self::path_to_string(&canonical)));
        Ok(id)
    }
    ...
    pub fn save_file(&mut self, doc_id: DocumentId) -> Result<(), LunaError> {
        let path = {
            let doc = self.get_doc_mut(doc_id)?;
            let path = doc.path().map(|p| Self::path_to_string(p))
                .ok_or_else(|| LunaError::Internal(format!("{} não tem caminho; use save_file_as", doc_id)))?;
            doc.save()?;
            path
        };
        self.emit_file_saved(doc_id, path);
    ...
    pub fn save_file_as(&mut self, doc_id: DocumentId, new_path: &str) -> Result<(), LunaError> {
        let emitted_path = {
            let doc = self.get_doc_mut(doc_id)?;
            doc.save_as(Path::new(new_path))?;
            Self::doc_path_to_string(doc).unwrap_or_else(|| new_path.to_owned())
    ...
    pub fn get_line(&self, doc_id: DocumentId, line: usize) -> Result<String, LunaError> {
        let doc = self.get_doc(doc_id)?;
        doc.get_line(line).map(|s| s.to_owned()).ok_or_else(|| {
            LunaError::invalid_pos(doc_id, Position::new(line, 0),
                format!("documento tem {} linhas", doc.line_count()))
        })
    }
    ...
    pub fn list_open_documents(&self) -> Vec<DocumentInfo> {
        let mut docs: Vec<DocumentInfo> = self.documents.values().map(|d| d.info()).collect();
        docs.sort_unstable_by_key(|d| d.id.0);
        docs
    }

    pub fn insert_text(&mut self, doc_id: DocumentId, position: Position, text: &str) -> Result<(), LunaError> {
        let range = Range::empty_at(position);
        let new_text = text.to_owned();
        { let doc = self.get_doc_mut(doc_id)?; doc.insert_text(position, text)?; }
        self.emit_text_changed(doc_id, range, new_text);
        Ok(())
    }

    pub fn delete_text(&mut self, doc_id: DocumentId, range: Range) -> Result<(), LunaError> {
        { let doc = self.get_doc_mut(doc_id)?; doc.delete_text(range)?; }
        self.emit_text_changed(doc_id, range, String::new());
        Ok(())
    }

    pub fn replace_text(&mut self, doc_id: DocumentId, range: Range, new_text: &str) -> Result<(), LunaError> {
        let nt = new_text.to_owned();
        { let doc = self.get_doc_mut(doc_id)?; doc.replace_text(range, new_text)?; }
        self.emit_text_changed(doc_id, range, nt);
        Ok(())
    }
    ...
    pub fn set_cursor(&mut self, doc_id: DocumentId, position: Position) -> Result<(), LunaError> {
        { let doc = self.get_doc_mut(doc_id)?; doc.set_cursor(position)?; }
        self.emit_cursor_moved(doc_id, position);
        Ok(())
    }
    ...
    pub fn execute_command(&self, name: &str, args: &[String]) -> Result<Option<String>, LunaError> {
        self.command_registry.execute(name, args)
    }
