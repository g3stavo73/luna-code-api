...
fn emit_text_changed(&self, doc_id: DocumentId, range: Range, new_text: String) {
    self.event_bus.emit(&Event::TextChanged {
        doc_id,
        range,
        new_text,
    });
...
pub fn save_file(&mut self, doc_id: DocumentId) -> Result<(), LunaError> {
    let path = {
        ...
        let path = doc
            ...
            .ok_or_else(|| {
                LunaError::Internal(format!(
                    "{} não tem caminho; use save_file_as",
                    doc_id
                ))
            })?;
...
pub fn replace_text(
    ...
    {
        let doc = self.get_doc_mut(doc_id)?;
        doc.replace_text(range, new_text)?;
    }
